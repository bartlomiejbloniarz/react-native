...
public class FabricUIManager
  ...
  private void scheduleMountItem(
    ...
    // When scheduling (push model), this is ~always called on the JS thread, or on the UI
    // thread in case of sync rendering. When executing synchronously (pull model), this is
    // always called on the UI thread, at pull time.
    ...
    if (shouldSchedule) {
      ...
      if (synchronous) {
        // Pull model: we are already on the UI thread, inside the dispatcher's loop executing a
        // PullTransactionMountItem; exceptions land in that loop's catch, same as any push batch.
   ...
   * is available for {@code surfaceId}. Enqueues a lightweight PullTransactionMountItem so the UI
   * thread pulls and applies the transaction itself, preserving mount-item ordering and the
