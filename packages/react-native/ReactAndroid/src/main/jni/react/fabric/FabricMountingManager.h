...
class FabricMountingManager final {
 public:
   ...
   * hands it to Java. When `synchronous` is false (push model), the batch is
   * scheduled onto the UI thread asynchronously. When `synchronous` is true
   * (pull model), the batch is applied immediately on the calling (UI) thread.
   */
   ...
   * the UI thread can pull it via a PullTransactionMountItem. Lightweight; does
   * not pull or build anything here.
   */
