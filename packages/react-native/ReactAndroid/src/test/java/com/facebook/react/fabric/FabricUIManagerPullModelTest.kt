 ...
 * queued mount items synchronously; notifications posted from a background thread stay queued
 * until the next UI-thread dispatch (in production, the dispatcher's frame callback).
 */
...
class FabricUIManagerPullModelTest {
  ...
  @Test
  fun onTransactionAvailable_withoutBinding_isNoOp() {
    val manager =
        FabricUIManager(
            reactContext,
            ViewManagerRegistry(emptyList()),
    ...
    // Must not throw and must not pull.
    manager.onTransactionAvailable(1)
